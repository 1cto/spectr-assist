...
export const ChatPanel = forwardRef<ChatPanelRef, ChatPanelProps>(
  ({ featureContent, onFeatureChange, sessionId, onStartWaiting }, ref) => {
    const { user } = useAuth();
          ...
          setMessages([
            {
              id: "1",
              content:
                "Hello! I'm here to help you create better feature files. What requirement would you like to work on?",
              sender: "assistant",
              timestamp: new Date(),
            },
          ]);
    ...
    const handleSend = async () => {
      ...
      try {
        ...
        const chatContent =
          webhookResponse && typeof webhookResponse === "object" && webhookResponse.output
            ? webhookResponse.output
            : typeof webhookResponse === "string"
              ? webhookResponse
              : "I received your message and processed it successfully.";
            ...
            {isLoadingHistory ? (
              ...
              messages.map((message) => (
                ...
                >
                  ...
                  <div
                    className={`max-w-[80%] p-3 rounded-lg ${message.sender === "user" ? "text-black" : ""}`}
