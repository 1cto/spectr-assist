...
const BITRIX_WEBHOOK_URL_BASE = Deno.env.get('BITRIX_WEBHOOK_URL_BASE');
...
serve(async (req) => {
  ...
  try {
    ...
    if (!searchData.result || searchData.result.length === 0) {
      ...
      return new Response(
        JSON.stringify({ success: false, message: `Lead not found for email: ${email}` }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 }
      );
    }
    ...
    return new Response(
      JSON.stringify({ success: true, leadId: leadId, result: updateData.result }),
      {
